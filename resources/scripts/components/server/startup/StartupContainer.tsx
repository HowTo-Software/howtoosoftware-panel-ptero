...
const StartupContainer = () => {
    ...
    return !data ? (
        !error || (error && isValidating) ? (
            ...
            <ServerError title={translateUiText('Oops!')} message={httpErrorToHuman(error)} onRetry={() => mutate()} />
        )
    ) : (
        <ServerContentBlock title={translateUiText('Startup Settings')} showFlashKey={'startup:image'}>
            <div className={styles.startupPage}>
                <div css={tw`md:flex`}>
                    <TitledGreyBox title={translateUiText('Startup Command')} css={tw`flex-1`}>
                        <div css={tw`px-1 py-2`}>
                            <p css={tw`font-mono bg-neutral-900 rounded py-2 px-4`}>{data.invocation}</p>
                        </div>
                    </TitledGreyBox>
                    <TitledGreyBox
                        title={translateUiText('Docker Image')}
                    ...
                    >
                        {Object.keys(data.dockerImages).length > 1 && !isCustomImage ? (
                            ...
                            <>
                                ...
                                {isCustomImage && (
                                    <p css={tw`text-xs text-neutral-300 mt-2`}>
                                        {translateUiText('This')} {translateUiText("server's")}{' '}
                                        {translateUiText(
                ...
                <h3 css={tw`mt-8 mb-2 text-2xl`}>{translateUiText('Variables')}</h3>
