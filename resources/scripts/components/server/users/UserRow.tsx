    ...
    return (
        <GreyRowBox css={tw`mb-2`}>
            ...
            <div css={tw`ml-4`}>
                ...
                <p css={tw`text-2xs text-neutral-500 uppercase hidden md:block`}>{translateUiText('2FA Enabled')}</p>
            </div>
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center`}>
                    {subuser.permissions.filter((permission) => permission !== 'websocket.connect').length}
                </p>
                <p css={tw`text-2xs text-neutral-500 uppercase`}>{translateUiText('Permissions')}</p>
            </div>
            {subuser.uuid !== uuid && (
                <>
                    <Can action={'user.update'}>
                        <button
                            type={'button'}
                            aria-label={translateUiText('Edit subuser')}
