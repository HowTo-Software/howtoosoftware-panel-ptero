...
const ActivePill = ({ active }: { active: boolean }) => (
    ...
    >
        {active ? translateUiText('Active') : translateUiText('Inactive')}
    </span>
);
...
export default () => {
    ...
    return (
        <PageContentBlock title={translateUiText('Schedules')} serverPage>
                ...
                <>
                    ...
                    <div css={tw`rounded shadow`}>
                        ...
                        >
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    {translateUiText('Last run at:')}
                                    {schedule.lastRunAt ? (
                                        format(schedule.lastRunAt, "MMM do 'at' h:mma")
                                    ) : (
                                        <span css={tw`text-neutral-300`}>{translateUiText('n/a')}</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        {translateUiText('Next run at:')}
                                        {schedule.nextRunAt ? (
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            <CronBox title={translateUiText('Minute')} value={schedule.cron.minute} />
                            <CronBox title={translateUiText('Hour')} value={schedule.cron.hour} />
                            <CronBox title={translateUiText('Day (Month)')} value={schedule.cron.dayOfMonth} />
                            <CronBox title={translateUiText('Month')} value={schedule.cron.month} />
                            <CronBox title={translateUiText('Day (Week)')} value={schedule.cron.dayOfWeek} />
                        </div>
