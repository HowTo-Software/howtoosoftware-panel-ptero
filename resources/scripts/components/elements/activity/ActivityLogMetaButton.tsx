...
export default ({ meta }: { meta: Record<string, unknown> }) => {
    ...
    return (
        <div className={'self-center md:px-4'}>
            <Dialog open={open} onClose={() => setOpen(false)} hideCloseIcon title={translateUiText('Metadata')}>
                <pre
                    className={
                ...
                <Dialog.Footer>
                    <Button.Text onClick={() => setOpen(false)}>{translateUiText('Close')}</Button.Text>
                </Dialog.Footer>
            </Dialog>
            <button
                aria-describedby={translateUiText('View additional event metadata')}
                className={
