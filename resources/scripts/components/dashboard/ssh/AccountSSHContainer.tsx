...
export default () => {
    ...
    return (
        <PageContentBlock title={translateUiText('SSH Keys')}>
            <FlashMessageRender byKey={'account'} />
            <div css={tw`md:flex flex-nowrap my-10`}>
                <ContentBox title={translateUiText('Add SSH Key')} css={tw`flex-none w-full md:w-1/2`}>
                    <CreateSSHKeyForm />
                </ContentBox>
                <ContentBox title={translateUiText('SSH Keys')} css={tw`flex-1 overflow-hidden mt-8 md:mt-0 md:ml-8`}>
                    <SpinnerOverlay visible={!data && isValidating} />
                    {!data || !data.length ? (
                        <p css={tw`text-center text-sm`}>
                            {!data
                                ? translateUiText('Loading...')
                                : translateUiText('No SSH Keys exist for this account.')}
                        </p>
                    ) : (
                        data.map((key, index) => (
                            <GreyRowBox
