...
@section('content-header')
    <h1>{{ __('Mounts') }}<small>{{ __('Configure and manage additional mount points for servers.') }}</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">{{ __('Admin') }}</a></li>
        <li class="active">{{ __('Mounts') }}</li>
    </ol>
@endsection
        ...
        <div class="col-xs-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Mount List') }}</h3>

                    <div class="box-tools">
                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#newMountModal">{{ __('Create New') }}</button>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.mounts') }}" method="POST">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ __('Create Mount') }}</h4>
                    </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <p class="text-muted small">{{ __('Unique name used to separate this mount from another.') }}</p>
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <p class="text-muted small">{{ __('A longer description for this mount, must be less than 191 characters.') }}</p>
                            </div>

                            <div class="col-md-6">
                                <label for="pSource" class="form-label">{{ __('Source') }}</label>
                                <input type="text" id="pSource" name="source" class="form-control" />
                                <p class="text-muted small">{{ __('File path on the host system to mount to a container.') }}</p>
                            </div>

                            <div class="col-md-6">
                                <label for="pTarget" class="form-label">{{ __('Target') }}</label>
                                <input type="text" id="pTarget" name="target" class="form-control" />
                                <p class="text-muted small">{{ __('Where the mount will be accessible inside a container.') }}</p>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="text-muted small">{{ __('Is the mount read only inside the container?') }}</p>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <p class="text-muted small">{{ __('Should users be able to mount this themselves?') }}</p>
                            </div>
                        </div>
                    </div>
