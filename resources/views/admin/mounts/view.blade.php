...
@section('title')
    {{ __('Mounts &rarr; View &rarr;') }} {{ $mount->id }}
@endsection
...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('admin.mounts') }}">{{ __('Mounts') }}</a></li>
...
@section('content')
    <div class="row">
        <div class="col-sm-6">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Mount Details') }}</h3>
                </div>

                <form action="{{ route('admin.mounts.view', $mount->id) }}" method="POST">
                    <div class="box-body">
                        <div class="form-group">
                            <label for="PUniqueID" class="form-label">{{ __('Unique ID') }}</label>
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="pSource" class="form-label">{{ __('Source') }}</label>
                                <input type="text" id="pSource" name="source" class="form-control" value="{{ $mount->source }}" />
                            </div>

                            <div class="form-group col-md-6">
                                <label for="pTarget" class="form-label">{{ __('Target') }}</label>
                    ...
                    <div class="box-footer">
                        ...
                        <button name="action" value="edit" class="btn btn-sm btn-primary pull-right">{{ __('Save') }}</button>
        ...
        <div class="col-sm-6">
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Eggs') }}</h3>

                    <div class="box-tools">
                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addEggsModal">{{ __('Add Eggs') }}</button>
                    </div>
                </div>
            ...
            <div class="box">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Nodes') }}</h3>

                    <div class="box-tools">
                        <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#addNodesModal">{{ __('Add Nodes') }}</button>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.mounts.eggs', $mount->id) }}" method="POST">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ __('Add Eggs') }}</h4>
                    </div>

                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                    <label for="pEggs">{{ __('Eggs') }}</label>
            ...
            <div class="modal-content">
                <form action="{{ route('admin.mounts.nodes', $mount->id) }}" method="POST">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">{{ __('Add Nodes') }}</h4>
                    </div>

                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-md-12">
                                <label for="pNodes">{{ __('Nodes') }}</label>
...
@section('footer-scripts')
    ...
    <script>
        $(document).ready(function() {
            $('#pEggs').select2({
                placeholder: @json(__('Select eggs..')),
            });

            $('#pNodes').select2({
                placeholder: @json(__('Select nodes..')),
            });
            ...
            $('button[data-action="detach-egg"]').click(function (event) {
                ...
                swal({ type: 'success', title: @json(__('Egg detached.')) });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
            ...
            $('button[data-action="detach-node"]').click(function (event) {
                ...
                $.ajax({
                    ...
                    element.parent().parent().addClass('warning').delay(100).fadeOut();
                swal({ type: 'success', title: @json(__('Node detached.')) });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
                    title: @json(__('Whoops!')),
