...
@section('title')
    {{ __('Locations &rarr; View &rarr;') }} {{ $location->short }}
@endsection
...
@section('content-header')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{ route('admin.locations') }}">{{ __('Locations') }}</a></li>
    ...
    <div class="col-sm-6">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __('Location Details') }}</h3>
            </div>
            <form action="{{ route('admin.locations.view', $location->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        <label for="pShort" class="form-label">{{ __('Short Code') }}</label>
                        <input type="text" id="pShort" name="short" class="form-control" value="{{ $location->short }}" />
                    </div>
                    <div class="form-group">
                        <label for="pLong" class="form-label">{{ __('Description') }}</label>
