...
<div class="row">
                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <div>
                            ...
                            <p class="text-muted"><small>{{ __('This should be a descriptive category name that encompasses all of the options within the service.') }}</small></p>
                        </div>
                    </div>
                    <div class="form-group">
    ...
    <div class="col-md-6">
        <div class="box">
            <div class="box-body">
                <div class="form-group">
                    <label class="control-label">{{ __('Nest ID') }}</label>
                    <div>
                        <input type="text" readonly class="form-control" value="{{ $nest->id }}" />
                        <p class="text-muted small">{{ __('A unique ID used for identification of this nest internally and through the API.') }}</p>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div>
                        ...
                        <p class="text-muted small">{{ __('The author of this service option. Please direct questions and issues to them unless this is an official option authored by') }} <code>support@pterodactyl.io</code>.</p>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div>
                        ...
                        <p class="text-muted small">{{ __('A UUID that all servers using this option are assigned for identification purposes.') }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __('Nest Eggs') }}</h3>
            </div>
            ...
            <div class="box-footer">
                <a href="{{ route('admin.nests.egg.new') }}"><button class="btn btn-success btn-sm pull-right">{{ __('New Egg') }}</button></a>
            </div>
        </div>
    </div>
</div>
...
@section('footer-scripts')
    ...
    <script>
        $('#deleteButton').on('mouseenter', function (event) {
            $(this).find('i').html(' ' + window.howTooTranslate('Delete Nest'));
