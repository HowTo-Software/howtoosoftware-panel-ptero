...
@section('title')
    {{ $node->name }}{{ __(': Allocations') }}
@endsection

@section('content-header')
    <h1>{{ $node->name }}<small>{{ __('Control allocations available for servers on this node.') }}</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">{{ __('Allocation') }}</a></li>
    ...
    <div class="col-sm-8">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __('Existing Allocations') }}</h3>
            </div>
            <div class="box-body table-responsive no-padding" style="overflow-x: visible">
                <table class="table table-hover" style="margin-bottom:0;">
                    <tr>
                        ...
                        <th>{{ __('IP Address') }} <i class="fa fa-fw fa-minus-square" style="font-weight:normal;color:#d9534f;cursor:pointer;" data-toggle="modal" data-target="#allocationModal"></i></th>
                        <th>{{ __('IP Alias') }}</th>
                        <th>{{ __('Port') }}</th>
                        ...
                        <th>
                            <div class="btn-group hidden-xs">
                                <button type="button" id="mass_actions" class="btn btn-sm btn-default dropdown-toggle disabled"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{ __('Mass Actions') }} <span class="caret"></span>
                                </button>
                                <ul class="dropdown-menu dropdown-massactions">
                                    <li><a href="#" id="selective-deletion" data-action="selective-deletion">{{ __('Delete') }} <i class="fa fa-fw fa-trash-o"></i></a></li>
                                </ul>
                            </div>
                        </th>
                    </tr>
                    @foreach($node->allocations as $allocation)
                        <tr>
                            ...
                            <td class="col-sm-3 middle">
                                <input class="form-control input-sm" type="text" value="{{ $allocation->ip_alias }}" data-action="set-alias" data-id="{{ $allocation->id }}" placeholder="{{ __('none') }}" />
    ...
    <div class="col-sm-4">
        <form action="{{ route('admin.nodes.view.allocation', $node->id) }}" method="POST">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Assign New Allocations') }}</h3>
                </div>
                <div class="box-body">
                    <div class="form-group allocation-multiselect">
                        <label for="pAllocationIP" class="control-label">{{ __('IP Address') }}</label>
                        <div>
                            ...
                            <p class="text-muted small">{{ __('Enter an IP address to assign ports to here.') }}</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationAlias" class="control-label">{{ __('IP Alias') }}</label>
                        <div>
                            <input type="text" id="pAllocationAlias" class="form-control" name="allocation_alias" placeholder="alias" />
                            <p class="text-muted small">{{ __('If you would like to assign a default alias to these allocations enter it here.') }}</p>
                        </div>
                    </div>
                    <div class="form-group allocation-multiselect">
                        <label for="pAllocationPorts" class="control-label">{{ __('Ports') }}</label>
                        <div>
                            <select class="form-control" name="allocation_ports[]" id="pAllocationPorts" multiple></select>
                            <p class="text-muted small">{{ __('Enter individual ports or port ranges here separated by commas or spaces.') }}</p>
                        </div>
                    </div>
                </div>
                <div class="box-footer">
                    {!! csrf_field() !!}
                    <button type="submit" class="btn btn-success btn-sm pull-right">{{ __('Submit') }}</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{ __('Delete Allocations for IP Block') }}</h4>
            </div>
            <form action="{{ route('admin.nodes.view.allocation.removeBlock', $node->id) }}" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger">{{ __('Delete Allocations') }}</button>
                </div>
            </form>
        </div>
    </div>
...
@section('footer-scripts')
    ...
    $('#pAllocationIP').select2({
        ...
        selectOnClose: true,
        tokenSeparators: [',', ' '],
                placeholder: @json(__('Select or enter an IP address')),
    ...
    $('#pAllocationPorts').select2({
        ...
        selectOnClose: true,
        tokenSeparators: [',', ' '],
                placeholder: @json(__('Enter ports or port ranges')),
        dropdownCssClass: 'hts-allocation-dropdown',
    });
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            title: '',
            text: @json(__('Are you sure you want to delete this allocation?')),
            type: 'warning',
            ...
            swal({ type: 'success', title: @json(__('Port Deleted!')) });
            }).fail(function (jqXHR) {
                console.error(jqXHR);
                swal({
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            swal({
                type: 'warning',
                title: '',
            text: @json(__('Are you sure you want to delete the following allocations: ')) + formattedItems + '?',
                html: true,
                showCancelButton: true,
                ...
                $.ajax({
                    ...
                    swal({
                        type: 'success',
            title: @json(__('Allocations Deleted'))
                    });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
                ...
                title: @json(__('Whoops!')),
                        html: true,
                text: @json(__('An error occurred while attempting to delete these allocations. Please try again.')),
                    });
                });
            });
        } else {
            swal({
                type: 'warning',
                title: '',
            text: @json(__('Please select allocation(s) to delete.')),
            });
        }
    }
    </script>
