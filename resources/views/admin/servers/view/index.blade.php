                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            <tr>
                                <td>{{ __('Internal Identifier') }}</td>
                                <td><code>{{ $server->id }}</code></td>
                            </tr>
                            <tr>
                                <td>{{ __('External Identifier') }}</td>
                                @if(is_null($server->external_id))
                                    <td><span class="label label-default">{{ __('Not Set') }}</span></td>
                                @else
                                    <td><code>{{ $server->external_id }}</code></td>
                                @endif
                            </tr>
                            <tr>
                                <td>{{ __('UUID / Docker Container ID') }}</td>
                                <td><code>{{ $server->uuid }}</code></td>
                            </tr>
                            <tr>
                                <td>{{ __('Current Egg') }}</td>
                                <td>
                            ...
                            <tr>
                                <td>{{ __('CPU Pinning') }}</td>
                                <td>
                                    @if($server->threads != null)
                                        <code>{{ $server->threads }}</code>
                                    @else
                                        <span class="label label-default">{{ __('Not Set') }}</span>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    /
                                    @if($server->swap === 0)
                                        <code data-toggle="tooltip" data-placement="top" title="{{ __('Swap Space') }}">{{ __('Not Set') }}</code>
                                    @elseif($server->swap === -1)
                                        <code data-toggle="tooltip" data-placement="top" title="{{ __('Swap Space') }}">{{ __('Unlimited') }}</code>
                                    @else
                                        <code data-toggle="tooltip" data-placement="top" title="{{ __('Swap Space') }}"> {{ $server->swap }}MiB</code>
                                    @endif
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td>{{ __('Block IO Weight') }}</td>
                                <td><code>{{ $server->io }}</code></td>
                            </tr>
                            <tr>
                                <td>{{ __('Default Connection') }}</td>
                                <td><code>{{ $server->allocation->ip }}:{{ $server->allocation->port }}</code></td>
                            </tr>
                            <tr>
                                <td>{{ __('Connection Alias') }}</td>
                                <td>
                                    @if($server->allocation->alias !== $server->allocation->ip)
                                        <code>{{ $server->allocation->alias }}:{{ $server->allocation->port }}</code>
                                    @else
                                        <span class="label label-default">{{ __('No Alias Assigned') }}</span>
                                    @endif
                                </td>
                            </tr>
                        </table>
                    ...
                    @if($server->isSuspended())
                        <div class="col-sm-12">
                            <div class="small-box bg-yellow">
                                <div class="inner">
                                    <h3 class="no-margin">{{ __('Suspended') }}</h3>
                                </div>
                            </div>
                        </div>
                    @endif
