...
@section('title')
    {{ __('Server —') }} {{ $server->name }}: {{ __('Mounts') }}
@endsection

@section('content-header')
    <h1>{{ $server->name }}<small>{{ __('Manage server mounts.') }}</small></h1>
    <ol class="breadcrumb">
        ...
        <div class="col-sm-12">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">{{ __('Available Mounts') }}</h3>
                </div>
                            ...
                            <tr>
                                ...
                                @if (! in_array($mount->id, $server->mounts->pluck('id')->toArray()))
                                    <td class="col-sm-2 middle">
                                        <span class="label label-primary">{{ __('Unmounted') }}</span>
                                    </td>
                                ...
                                @else
                                    <td class="col-sm-2 middle">
                                        <span class="label label-success">{{ __('Mounted') }}</span>
                                    </td>
