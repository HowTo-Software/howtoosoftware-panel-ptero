...
@section('title')
    {{ __('Server —') }} {{ $server->name }}: {{ __('Details') }}
@endsection

@section('content-header')
    <h1>{{ $server->name }}<small>{{ __('Edit details for this server including owner and container.') }}</small></h1>
    <ol class="breadcrumb">
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">{{ __('Base Information') }}</h3>
            </div>
            <form action="{{ route('admin.servers.view.details', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        <label for="name" class="control-label">{{ __('Server Name') }} <span class="field-required"></span></label>
                        <input type="text" name="name" value="{{ old('name', $server->name) }}" class="form-control" />
                        <p class="text-muted small">{{ __('Character limits:') }} <code>a-zA-Z0-9_-</code> {{ __('and') }} <code>[Space]</code>.</p>
                    </div>
                    <div class="form-group">
                        <label for="external_id" class="control-label">{{ __('External Identifier') }}</label>
                        <input type="text" name="external_id" value="{{ old('external_id', $server->external_id) }}" class="form-control" />
                        <p class="text-muted small">{{ __('Leave empty to not assign an external identifier for this server. The external ID should be unique to this server and not be in use by any other servers.') }}</p>
                    </div>
                    <div class="form-group">
                        <label for="pUserId" class="control-label">{{ __('Server Owner') }} <span class="field-required"></span></label>
                        <select name="owner_id" class="form-control" id="pUserId">
                            <option value="{{ $server->owner_id }}" selected>{{ $server->user->email }}</option>
                        </select>
                        <p class="text-muted small">{{ __('You can change the owner of this server by changing this field to an email matching another use on this system. If you do this a new daemon security token will be generated automatically.') }}</p>
                    </div>
                    <div class="form-group">
                        <label for="description" class="control-label">{{ __('Server Description') }}</label>
