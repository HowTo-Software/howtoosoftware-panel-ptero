...
class IntegrationSettingsFormRequest extends AdminFormRequest
{
    public function rules(): array
    {
        ...
        foreach (IntegrationCredentialStore::PROVIDERS as $provider) {
            ...
            $rules["providers.$provider.timeout_seconds"] = ['required', 'integer', 'min:5', 'max:180'];
            ...
            $rules["providers.$provider.base_url"] = $provider === 'ollama'
                ? ['nullable', 'url:http,https', 'max:255', 'regex:~^https?://[^/?#]+/?(?:api/?)?$~i']
                : ['nullable', Rule::in([''])];
