...
class LocaleController extends Controller
{
    ...
    {
        $locales = $this->splitBatch($request->string('locale')->toString());
        $namespaces = $this->splitBatch($request->string('namespace')->toString());
        $response = [];
    ...
    /**
     * The i18next multiload adapter joins values with `+`. PHP decodes a plus
     * in a query string as whitespace, so accept both representations here.
     */
    private function splitBatch(string $value): array
    {
        return array_values(array_unique(preg_split('/[+\s]+/', $value, -1, PREG_SPLIT_NO_EMPTY)));
    }
